...
int main()
{
	...
	int shmidMemoria;
	char *path="/home/smartmanager/Downloads/uEye_SimpleSingleGrab";
	int *contEncoder=0;
	...
	*contEncoder = 0;
	...
	while(1){
		...
		switch (Estado){
			...
			case 0:
				if (gpio_get_value(298)==1 && gpio_get_value(480)==0){ //Aguarda gaveta completamente aberta
					cont1++;
					if (cont1>=10){
						cont1 = 0;
						Estado = 2;}}
			...
			case 2:
				while(gpio_get_value(298)==1); //Aguarda início do movimento de fechamento da gaveta
				...
				do{
					...
					if(gpio_get_value(480)==1) //Gaveta completamente fechada
						cont2++;
					else
						cont2=0;
