...
int main()
{
    ...
    *ImagemCam1OK = 0;
    ...
    int ImgWidth = 1248; //Max 1448
    int regiaoScan = 2;
    int ImgHeight = 4; //Max 1086 //100 obs. deve resultar em um número par
    int OffsetX = 50;
    int OffsetY = 1086-795; //obs. o valor é reduzido de 1086 por que a câmera está invertida verticalmente
    ...
    double shutter=0.5; //Tempo de exposição em ms
    ...
    templMatching = imread("RedesGV1/1.png", CV_32FC1);
	...
	imgMatching = resized(cv::Rect(x, y, width, height)).clone();
    templMatching = imread("RedesGV1/2.png", CV_32FC1);
	...
	imgMatching = resized(cv::Rect(x, y, width, height)).clone();
    templMatching = imread("RedesGV1/6.png", CV_32FC1);
	...
	imgMatching = resized(cv::Rect(x, y, width, height)).clone();
    templMatching = imread("RedesGV1/7.png", CV_32FC1);
	...
	imgMatching = resized(cv::Rect(x, y, width, height)).clone();
    templMatching = imread("RedesGV1/8.png", CV_32FC1);
