...
int main()
{
    ...
    *ImagemCam1OK = 0;
    ...
    int OffsetX = 330;
    int OffsetY = 1086-560; //obs. o valor é reduzido de 1086 por que a câmera está invertida verticalmente
    ...
    double shutter=2; //Tempo de exposição em ms
	...
	do{
		if (*contEncoder>=ProximoRegistro){
			...
			ProximoRegistro = ProximoRegistro+16;
			cont++;
    ...
    // Salva imagem
	std::string name = "GV8_cam1.png"; //Define endereço das imagens
	imwrite(name, image1F);

	cv::Mat resized;
	resize(image1F, resized, Size(ImgWidth/2, 420/2), 0, 0, INTER_CUBIC);
	name = "GV8_cam1_resized.png";
