...
jetsonXavierGPIONumber ligaLampada = gpio491; //Pino liga iluminação - pino fisico 23
jetsonXavierGPIONumber encoderChA = gpio481; //Pino pulso encoder - pino fisico 22
jetsonXavierGPIONumber encoderChB = gpio494; //Pino pulso encoder - pino fisico 24
jetsonXavierGPIONumber bateriaBIT0 = gpio421;//gpio448; //Pino nivel bateria bit 0 - pino fisico 35
...
jetsonXavierGPIONumber enableGavetas = gpio447; //Pino habilita gavetas - pino fisico 38
jetsonXavierGPIONumber enablePWM = gpio393;
...
void Motores(char DATA[4]) //Movimento dos Motores
{
  ...
  *contEncoder=0;
  ...
  if((strncmp (DATA, "AGV", 3) == 0))
  {
    ...
    gpioSetValue(enableGavetas, on);
    //gpioSetValue(enablePWM, on);
    ...
    while (cont <= 3){
      ...
      if (contEncoderAbertura<200 && gpioGetValue(sobrecarga)==0){
        cont4++;
        if (cont4>=1000){
          gavetaTrancada = 1;
          cout << "GVT";
    ...
    gpioSetValue(enableGavetas, off);
    //gpioSetValue(enablePWM, off);
    if (gavetaTrancada==0)
  ...
  if ((strncmp (DATA, "FGV", 3) == 0))
  {
    ...
    while (cont <= 5){
      ...
      //Verifica gaveta trancada
      if (*contEncoder<200 && gpioGetValue(sobrecarga)==0){
        cont4++;
        if (cont4>=1200){
          gavetaTrancada = 1;
          cout << "GVT";
...
int main(int argc, char *argv[]){
  ...
  gpioExport(enableGavetas);
  gpioSetDirection(enableGavetas,outputPin);
  gpioExport(enablePWM);
  gpioSetDirection(enablePWM,outputPin);
  ...
  else if (select=="BATERIA"){ //Retorna nível da bateria
    NivelBateria();
  }
