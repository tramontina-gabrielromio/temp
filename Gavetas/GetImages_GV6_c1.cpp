...
int main()
{
    ...
    *ImagemCam1OK = 0;
    ...
    int ImgWidth = 408; //Max 1448
    int regiaoScan = 2;
    int ImgHeight = 4; //Max 1086 //100 obs. deve resultar em um número par
    int OffsetX = 450;
    int OffsetY = 1086-600; //obs. o valor é reduzido de 1086 por que a câmera está invertida verticalmente
	...
	do{
		if (*contEncoder>=ProximoRegistro){
			...
			ProximoRegistro = ProximoRegistro+13;
			cont++;
    ...
    // Salva imagem
	std::string name = "GV6_cam1.png"; //Define endereço das imagens
	imwrite(name, image1F);

	cv::Mat resized;
	resize(image1F, resized, Size(ImgWidth/2, 540/2), 0, 0, INTER_CUBIC);
	name = "GV6_cam1_resized.png";
	...
	imgMatching = resized(cv::Rect(x, y, width, height)).clone();
    templMatching = imread("RedesGV6/13.png", CV_32FC1);
    cv::Mat tool13 = MatchingMethod(imgMatching, templMatching).clone();
    cv::resize(tool13, tool13, cv::Size(44, 44));
	imwrite("ToolsGV6/Tool13.png", tool13);
	...
	imgMatching = resized(cv::Rect(x, y, width, height)).clone();
    templMatching = imread("RedesGV6/14.png", CV_32FC1);
    cv::Mat tool14 = MatchingMethod(imgMatching, templMatching).clone();
    cv::resize(tool14, tool14, cv::Size(44, 44));
	imwrite("ToolsGV6/Tool14.png", tool14);
	...
	imgMatching = resized(cv::Rect(x, y, width, height)).clone();
    templMatching = imread("RedesGV6/15.png", CV_32FC1);
    cv::Mat tool15 = MatchingMethod(imgMatching, templMatching).clone();
    cv::resize(tool15, tool15, cv::Size(44, 44));
	imwrite("ToolsGV6/Tool15.png", tool15);
	...
	imgMatching = resized(cv::Rect(x, y, width, height)).clone();
    templMatching = imread("RedesGV6/16.png", CV_32FC1);
    cv::Mat tool16 = MatchingMethod(imgMatching, templMatching).clone();
    cv::resize(tool16, tool16, cv::Size(44, 44));
	imwrite("ToolsGV6/Tool16.png", tool16);
