...
int main()
{
    ...
    *ImagemCam2OK = 0;
    ...
    // Salva imagem
	std::string name = "GV6_cam2.png"; //Define endereço das imagens
	imwrite(name, image1F);

	cv::Mat resized;
	resize(image1F, resized, Size(ImgWidth/2, 540/2), 0, 0, INTER_CUBIC);
	name = "GV6_cam2_resized.png";
	...
	imgMatching = resized(cv::Rect(x, y, width, height)).clone();
    templMatching = imread("RedesGV6/17.png", CV_32FC1);
    cv::Mat tool17 = MatchingMethod(imgMatching, templMatching).clone();
    cv::resize(tool17, tool17, cv::Size(44, 44));
	imwrite("ToolsGV6/Tool17.png", tool17);
	...
	imgMatching = resized(cv::Rect(x, y, width, height)).clone();
    templMatching = imread("RedesGV6/18.png", CV_32FC1);
    cv::Mat tool18 = MatchingMethod(imgMatching, templMatching).clone();
    cv::resize(tool18, tool18, cv::Size(44, 44));
	imwrite("ToolsGV6/Tool18.png", tool18);
	...
	imgMatching = resized(cv::Rect(x, y, width, height)).clone();
    templMatching = imread("RedesGV6/19.png", CV_32FC1);
    cv::Mat tool19 = MatchingMethod(imgMatching, templMatching).clone();
    cv::resize(tool19, tool19, cv::Size(44, 44));
	imwrite("ToolsGV6/Tool19.png", tool19);
